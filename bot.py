...
class AudienceMemberSelect(discord.ui.View):
    def __init__(self, post_content: str, image: discord.Attachment = None):
    ...
    async def select_audience(self, interaction: discord.Interaction, select: discord.ui.UserSelect):
        # Tell Discord to wait immediately so it doesn't time out
        ...
        # Attach the image to the private thread post if one exists
        if self.image:
        ...
        # Confirm success
        await interaction.edit_original_response(content=f"Private thread created successfully! Check {thread.mention}", view=None)
...
class PostTypeView(discord.ui.View):
    ...
    async def publicize_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # If an image was uploaded, process it and attach it to the public embed
        if self.image:
            file = await self.image.to_file()
            embed.set_image(url=f"attachment://{file.filename}")
    ...
    async def private_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Swap out the buttons for the User Select dropdown, passing the text and image forward
        view = AudienceMemberSelect(self.post_content, self.image)
        await interaction.response.edit_message(content="Who do you want to invite to see this post?", view=view)


# ==========================================
# 4. THE SLASH COMMAND & ERROR HANDLER
# ==========================================
@bot.tree.command(name="post", description="Create a post and choose who can see it")
@discord.app_commands.checks.has_any_role('Admin', 'Moderator', 'Vibe Manager') 
async def create_post(interaction: discord.Interaction, message: str, image: discord.Attachment = None):
    view = PostTypeView(message, image)
    await interaction.response.send_message("How would you like to share this post?", view=view, ephemeral=True)

# Handles the error if a normal member tries to use the command
@create_post.error
async def post_error(interaction: discord.Interaction, error: discord.app_commands.AppCommandError):
    if isinstance(error, discord.app_commands.errors.MissingAnyRole):
        await interaction.response.send_message(
            "⚠️ You do not have the required role to use this command!", 
            ephemeral=True
        )
